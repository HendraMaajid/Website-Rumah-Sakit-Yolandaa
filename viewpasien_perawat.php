<?php
    ...
    if(isset($_GET['id'])){
        $id = $_GET['id'];
        $date = date("Y-m-d");
        $sql = "UPDATE pesan_kamar SET tgl_keluar = '$date' WHERE no_rekam_medis = '$id' AND tgl_keluar IS NULL";
        $query = mysqli_query($conn, $sql);
        $sql = "DELETE FROM merawat WHERE no_rekam_medis = '$id'";
        $query = mysqli_query($conn, $sql);
        if($query){
            ?>
            <script>
                alert("pasien ini sudah pulang");
                location = "viewpasien_perawat.php";
            </script>
            <?php
        }
    }
            ...
            <div class="isi_content">
                ...
                <div class="search-container">
                    <form id="searchForm" method="get" action="">
                        ...
                        <table class="tabeldata" id="pasienTable">
                            <tr>
                                ...
                                if(mysqli_num_rows($q) > 0){
                                    while($row1=mysqli_fetch_array($q)){
                                        ...
                                        echo "<td><a class='link-delete'  href='viewpasien_perawat.php?id=$row1[no_rekam_medis]'>selesai</a></td>";
                                        echo "</tr>";
                                    }
                                }else{
                                    echo "<tr>";
...
<script>
    $(document).ready(function () {
        $('#search').on('input', function () {
            ...
            $.ajax({
                url: 'ajax/ajax_search_pasien_perawat.php',
                type: 'GET',
                ...
                success: function (data) {
                    ...
                    $('#pasienTable').html(tableWithHeaders);
                }
            });
        });
    });
