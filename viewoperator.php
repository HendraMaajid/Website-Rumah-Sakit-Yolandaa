    ...
    if(isset($_GET['id_delete'])){
        ...
        if($query){
            ...
            <script>
                ...
                document.location = "viewoperator.php";
            </script>
            <?php
        }else{
            ?>
            <script>
                alert("Data operator gagal dihapus");
                document.location = "viewoperator.php";
            </script>
            <?php
        }
            ...
            <div class="isi_content">
                ...
                <div class="search-container">
                    <form id="searchForm" method="get" action="">
                        ...
                        <table class="tabeldata" id="operatorTable">
                            <tr>
                                ...
                                if(mysqli_num_rows($q) > 0){
                                    while($row1=mysqli_fetch_array($q)){
                                        ...
                                        //buatkan link untuk edit dan hapus digabung 1
                                    ...
                                    echo "<tr>";
                                    echo "<td colspan='4'>Tidak ada operator yang terdaftar</td>";
                                    echo "</tr>";
                                }
                            ?>
                        </table>
                        <a href="insertoperator.php" ><button class="pesan" >Tambah Data</button></a>
            </div>
        </div>
        <div class="footer">
            <!-- nunggu homepage -->
    ...
    <script>
        $(document).ready(function () {
            $('#search').on('input', function () {
                ...
                $.ajax({
                    url: 'ajax/ajax_search_operator.php',
                    type: 'GET',
                    data: { search: searchQuery },
                    success: function (data) {
                        // Include table headers in the result
                        var tableWithHeaders = '<tr>' +
                            '<th class="table-header">Nama Operator</th>' +
                            '<th class="table-header">Tanggal Lahir</th>' +
                            '<th class="table-header">Alamat</th>' +
                            '<th class="table-header">No Hp</th>' +
                            '<th class="table-header">Email</th>' +
                        ...
                        $('#operatorTable').html(tableWithHeaders);
                    }
                });
            });
        });
