<?php
    ...
    if(isset($_POST["pesan"])){
        ...
        $sql_check = "SELECT * FROM konsultasi WHERE id_dokter = '$dokter' AND jam_konsultasi = '$jam' AND tgl_konsultasi = '$tanggal'";
        ...
        if($row_check){
            ...
            <script>
                ...
                document.location="bookingkonsultasi.php";
            </script>
            <?php
        }else{
            $sql = "INSERT INTO konsultasi (id_dokter, no_rekam_medis, keluhan, tgl_konsultasi, jam_konsultasi) VALUES ('$dokter', '$no_rekam_medis', '$keluhan', '$tanggal', '$jam')";
            $query = mysqli_query($conn, $sql);
             $sql = "INSERT INTO merawat (no_rekam_medis, jam_datang, tgl_masuk_rawat, id_perawat) 
            VALUES ('$no_rekam_medis', '$jam', '$tanggal', (SELECT id_perawat FROM perawat ORDER BY RAND() LIMIT 1))";
            $query = mysqli_query($conn, $sql);
            if($query){
                ?>
                <script>
            ...
            <div class="isi_content">
                ...
                <div class="kotak-biodata">
                    <form action="<?php $_SERVER['PHP_SELF']; ?>" method="post" >
                        <label for="kamar">Dokter :</label> <br>
                            <div class="custom-select" style="width:245px;">
                                <select name="dokter" required>
                                    ...
                                    while($row=mysqli_fetch_array($q)){
                                        echo "<option value='$row[id_dokter]'>$row[nama_dokter]-$row[spesialis]</option>";
                                    }
                                    ?>
                                </select>
                            </div>
