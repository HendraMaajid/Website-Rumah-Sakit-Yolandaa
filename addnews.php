<?php
    ...
    if(isset($_POST['tambah'])){
        ...
        if ($row['role'] == 'owner') {
        $id_pembuat = $row['id_owner'];
        } else {
            $id_pembuat = $row['id_operator'];
        }
        $sql = "INSERT INTO news (judul, konten, foto, id_pembuat) VALUES ('$judul', '$isi', '$upload','$id_pembuat')";
                ...
                <h1>Tambah Berita</h1>
                    <form action="<?php $_SERVER['PHP_SELF']; ?>" method="post" enctype="multipart/form-data" >
                       <label for="judul">Judul Berita:</label>
                        ...
                        <input type="submit" value="Tambah Berita" class="tambahberita" name="tambah">
                    </form>
                    <a href="news.php" class='view-all' >Lihat Berita</a>
            </div>
        </div>
        <div class="footer">
            <!-- nunggu homepage -->
