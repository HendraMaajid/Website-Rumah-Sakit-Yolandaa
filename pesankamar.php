<?php
    ...
    if(isset($_POST["pesan"])){
        ...
        $id_pasien = $_SESSION["no_rekam_medis"];
        $read = "SELECT * FROM pesan_kamar WHERE id_ruang = '$id_ruang' AND tgl_keluar IS NULL";
        $query = mysqli_query($conn, $read);
        if(mysqli_num_rows($query) > 0){
            ?>
            <script>
                alert("Kamar sudah dipesan");
                document.location="pesankamar.php";
            </script>
            <?php
        }else{
            $sql = "INSERT INTO pesan_kamar (id_ruang, tgl_masuk, no_rekam_medis) VALUES ('$id_ruang', '$tanggal_masuk', '$id_pasien')";
            $query = mysqli_query($conn, $sql);
            if($query){
                ?>
                <script>
                    alert("Anda berhasil memesan kamar");
                    document.location="pesankamar.php";
                </script>
                <?php
            }
                ...
                ?>
                <script>
                    alert("Anda gagal memesan kamar");
                    document.location="pesankamar.php";
                </script>
                <?php
            }
            ...
            <div class="isi_content">
                ...
                <div class="kotak-biodata">
                    <form action="<?php $_SERVER['PHP_SELF']; ?>" method="post" >
                        <label for="kamar">Nama Kamar :</label> <br>
                        ...
                        <input type="date" id="tanggal" name="tanggal_masuk" class="tanggal"> <br>
                        <button type="submit" class="pesan" name="pesan">Pesan</button>
                    </form>
                    <?php
                        $s = "SELECT * FROM pasien
                              JOIN pesan_kamar ON pasien.no_rekam_medis = pesan_kamar.no_rekam_medis
                              JOIN ruang ON pesan_kamar.id_ruang = ruang.id_ruang WHERE pasien.no_rekam_medis = '$_SESSION[no_rekam_medis]'
                        ...
                        if(mysqli_num_rows($q) > 0){
                            ...
                            <tr>
                                ...
                                <th class="table-header">Jenis Ruang</th>
                                <th class="table-header">Tanggal Masuk</th>
                            </tr>
                                <?php 
                                $now=date("Y-m-d");
                                $time=date("H:i:s");
