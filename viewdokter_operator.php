<?php
   ...
   if (!isset($_SESSION['role']) || ($_SESSION['role'] != "operator" && $_SESSION['role'] != "owner")) {
    ...
    if(isset($_GET['id_delete'])){
        $id = $_GET['id_delete'];
        $read = "SELECT * FROM dokter WHERE id_dokter = '$id'";
        $query = mysqli_query($conn, $read);
        $row = mysqli_fetch_array($query);
        $gambar = $row['foto'];
        unlink($gambar);
        ...
        if($query){
            ...
            <script>
                ...
                document.location = "viewdokter_operator.php";
            </script>
            <?php
        }else{
            ?>
                        ...
                        <table class="tabeldata" id="dokterTable">
                            <tr>
                                ...
                                if(mysqli_num_rows($q) > 0){
                                    ...
                                    echo "<tr>";
                                    echo "<td colspan='4'>Tidak ada dokter yang terdaftar</td>";
                                    echo "</tr>";
                                }
                            ?>
                        </table>
                        <a href="insertdokter.php" ><button class="pesan" >Tambah Data</button></a>
            </div>
        </div>
        <div class="footer">
            <!-- nunggu homepage -->
    ...
    <script>
        $(document).ready(function () {
            $('#search').on('input', function () {
                ...
                $.ajax({
                    url: 'ajax/ajax_search_dokter.php',
                    type: 'GET',
                    ...
                    success: function (data) {
                        ...
                        $('#dokterTable').html(tableWithHeaders);
                    }
                });
            });
        });
