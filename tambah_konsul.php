<?php
    ...
    if(isset($_POST["pesan"])){
        ...
        $jam = $_POST["jam"];
        $sql_check = "SELECT * FROM konsultasi WHERE id_dokter = '$row[id_dokter]' AND jam_konsultasi = '$jam' AND tgl_konsultasi = '$tanggal'";
        $query_check = mysqli_query($conn, $sql_check);
        $row_check = mysqli_fetch_array($query_check);
        if($row_check){
            ?>
            <script>
                alert("Jam tersebut sudah dipilih oleh pasien lain");
                document.location="tambah_konsul.php";
            </script>
            <?php
        }else{
            $sql = "INSERT INTO konsultasi (id_dokter, no_rekam_medis, keluhan, tgl_konsultasi, jam_konsultasi) VALUES ('$row[id_dokter]', '$pasien', '$keluhan', '$tanggal', '$jam')";
            $query = mysqli_query($conn, $sql);
            $sql = "INSERT INTO merawat (no_rekam_medis, jam_datang, tgl_masuk_rawat, id_perawat) 
            VALUES ('$pasien', '$jam', '$tanggal', (SELECT id_perawat FROM perawat ORDER BY RAND() LIMIT 1))";
            $query = mysqli_query($conn, $sql);

            if($query){
                ?>
                <script>
                    alert("Anda berhasil memesan konsultasi");
                    document.location="viewpasien_dokter.php";
                </script>
                <?php
            }
            else{
                ?>
                <script>
                    alert("Anda gagal memesan konsultasi");
                    document.location="tambah_konsul.php";
                </script>
                <?php
            }
            ...
            <div class="isi_content">
                ...
                <div class="kotak-biodata">
                    <form action="<?php $_SERVER['PHP_SELF']; ?>" method="post" >
                        <label for="kamar">Pasien :</label> <br>
                            <div class="custom-select" style="width:245px;">
                                <select name="pasien" required>
                                    <option value="0">Pilih pasien</option>
                                    <?php
                                    $s = "SELECT DISTINCT pasien.*
                                            FROM pasien
                                            JOIN konsultasi ON pasien.no_rekam_medis = konsultasi.no_rekam_medis
                                            JOIN dokter ON konsultasi.id_dokter = dokter.id_dokter
                                            WHERE dokter.id_dokter = '$row[id_dokter]'; ";
                                    $q = mysqli_query($conn, $s);
                                    while($row1=mysqli_fetch_array($q)){
                                        echo "<option value='$row1[no_rekam_medis]'>$row1[nama_pasien]</option>";
                                    }
                                    ?>
                                </select>
                            </div>
                        <label for="keluhan">Keluhan :</label> <br>
                        <input type="text" id="keluhan" name="keluhan" class="input-text" style="width:245px;"> <br>
                        <label for="tanggal">Tanggal Konsultasi :</label> <br>
                        <input type="date" id="tanggal" name="tanggal" class="tanggal" style="width:245px;"> <br>
                        <label for="jam">Jam Konsultasi :</label> <br>
                            <div class="custom-select" style="width:245px;">
                        ...
                        <button type="submit" class="pesan" name="pesan">Pesan</button>
                    </form>
                </div>
            </div>
        </div>
