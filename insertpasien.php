<?php
    ...
    if (!isset($_SESSION['role']) || ($_SESSION['role'] != "operator" && $_SESSION['role'] != "owner"  && $_SESSION['role'] != "pasien")) {
        header("Location: login.php");
        exit();
    }
    ...
    if(isset($_POST['simpan'])){
        ...
        if($count == 1){
            ...
            $sql = "INSERT INTO pasien (kis, nama_pasien, alamat, tanggal_lahir, email, no_hp, foto) VALUES ('$kis', '$nama', '$alamat', '$tanggal_lahir', '$email', '$no_hp', '$upload')";
            $query = mysqli_query($conn, $sql);
            ...
            $query = mysqli_query($conn, $sql);
            $sql = "INSERT INTO merawat (no_rekam_medis, jam_datang, tgl_masuk_rawat, id_perawat) VALUES ('$no_rekam_medis', '$time', '$date', (SELECT id_perawat FROM perawat ORDER BY RAND() LIMIT 1))";
            $query = mysqli_query($conn, $sql);
            $sql = "INSERT INTO pesan_kamar (no_rekam_medis, id_ruang, tgl_masuk) VALUES ('$no_rekam_medis','$ruang','$date')";
            $query = mysqli_query($conn, $sql);
            if($query){
                    ?>
                <script>
                    alert("Data pasien berhasil di tambahkan");
                    location = "viewpasien_operator.php";
                </script>
                <?php
            }else{
                ?>
...
<head>
    ...
    <link type="text/css" rel="stylesheet" href="style/insertpasien.css">
</head>
<body>
    <div class="container">
        <div class="content">
            ...
            <div class="isi_content">
                ...
                <h1>Tambah Pasien</h1>
                <form action="<?php $_SERVER['PHP_SELF']; ?>" method="post" name="insertPasien" enctype="multipart/form-data">
                    <label for="nama">Nama</label><br>
                    <input type="text" name="nama" id="nama"><br>

                    <label for="kis">KIS</label><br>
                    <input type="number" name="kis" id="kis" maxlength="16" ><br>
                    ...
                    <input type="file" name="foto" id="foto" accept=".jpg, .png"><br>
                    ...
                    <input type="submit" value="Insert" name="simpan" class="tambahberita">
                </form>
